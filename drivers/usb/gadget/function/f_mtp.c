...
/* huaqin add for ZB632KL by lihua at 2018/09/21 start */
	if (dev->state == STATE_OFFLINE) {
		spin_unlock_irq(&dev->lock);
		return -ENODEV;
	}
/* huaqin add for ZB632KL by lihua at 2018/09/21 end */
	if (dev->ep_out->desc) {
		/* huaqin add for ZB632KL by lihua at 2018/09/21 start */
		if (!cdev) {
			spin_unlock_irq(&dev->lock);
...
mtp_function_unbind(struct usb_configuration *c, struct usb_function *f)
{
	...
	mutex_unlock(&dev->read_mutex);
/* huaqin modify for ZB632KL by lihua at 2018/09/21 start */
	spin_lock_irq(&dev->lock);
	dev->state = STATE_OFFLINE;
	dev->cdev = NULL;
	spin_unlock_irq(&dev->lock);
